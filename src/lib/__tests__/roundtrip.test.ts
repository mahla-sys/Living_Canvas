...
/* ============================================================
   End-to-end at the storage layer: write → export → import → read
   (no React, no IndexedDB; exactly the functions production calls)
   ============================================================ */
...
const nodeData = {
  ...
  agent: {
    ...
    system_prompt:
      "You are the \"Understand the problem\" agent. Your mission is to read the canvas summary and your own memory and make the core problem explicit. Always list the ambiguous questions first, then write the problem statement as one precise paragraph. Your output must contain summary, problem_statement and questions_asked.",
    model: "deepseek-chat",
...
async function seedWorkspace() {
  ...
  await store.writeFile(`${ROOT}/canvas.yaml`, toYaml({ id: CANVAS_ID, title: "Nexus Smart School", owner: "mahla", canvas_type: "agent-pipeline", tags: ["nexus", "school"] }));
  await store.writeFile(`${ROOT}/canvas-overview.md`, frontmatter({ canvas_id: CANVAS_ID, node_count: 2, current_step: "Understand the problem" }, "# summary"));
  await store.writeFile(`${ROOT}/nodes/node-001.md`, nodeToMarkdown("node-001", nodeData, { x: 340, y: 250 }));
  await store.writeFile(`${ROOT}/nodes/node-002.md`, nodeToMarkdown("node-002", { ...nodeData, nodeType: "output-box", title: "Output box", agent: null } as never, { x: 900, y: 250 }));
  await store.writeFile(`${ROOT}/edges/edge-001.yaml`, edgeToYaml("edge-001", "node-001", "node-002", edgeData));
  await store.writeFile(`${ROOT}/memory/global.md`, memoryToMd({ path: "memory/global.md", title: "Overall status", body: "- goal: school", updated_at: "2026-09-01T10:00:00.000Z", last_accessed: "2026-09-01T10:00:00.000Z", confidence: 0.9, source: "system" }));
  await store.writeFile(`${ROOT}/memory/agents/node-001.md`, memoryToMd({ path: "memory/agents/node-001.md", title: "Agent memory", body: "- decision: continue", updated_at: "2026-09-01T10:00:00.000Z", last_accessed: "2026-09-01T10:00:00.000Z", confidence: 0.66, source: "agent" }));
  ...
  // a custom user template — exactly what the listDirectory bug used to lose
  await store.writeJson(`${ROOT}/library/templates/my-flow/template.json`, { template_id: "my-flow", name: "My flow", description: "d", version: "1.0", nodes: [], edges: [] });
  return store;
}

describe("workspace file tree", () => {
  it("hydration from files: every node/edge/memory that was written is read back", async () => {
    ...
    expect(d.nodes[0].data.title).toBe("Understand the problem");
    expect(d.nodes[0].position).toEqual({ x: 340, y: 250 });
    expect(d.nodes[0].data.agent?.system_prompt).toBe(nodeData.agent!.system_prompt); // no 120-char truncation
    ...
    expect(d.edges[0].data.label).toBe("problem statement");
    expect(d.canvasTitle).toBe("Nexus Smart School");
    expect(d.memory.global?.body).toContain("goal: school");
    expect(d.memory.agents["node-001"]?.confidence).toBe(0.66);
    expect(d.unreadable).toEqual([]);
  });

  it("template recovery bug: listDirectory returns the template folder and the spec is read", async () => {
    const store = await seedWorkspace();
...
describe("export → import roundtrip", () => {
  it("the JSON bundle returns the files byte for byte", async () => {
    const store = await seedWorkspace();
    ...
    /* comparing at the data-model level — timestamped fields (updated_at) are deliberately
       dropped, because makeNodeData stamps "now" on every derive. */
  ...
  it("an export without graph.json/state.json still builds the canvas (the Obsidian/Git path)", async () => {
    const store = await seedWorkspace();
    const collected: Record<string, string> = {};
    for (const p of await store.allPaths()) collected[p] = await store.readFile(p);
